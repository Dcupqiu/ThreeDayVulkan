...
#define VULKAN_XSTR(s) VULKAN_STR(s)
#define VULKAN_STR(s) #s
...
namespace VulkanManager
{
    ...
    void VulkanRHI::initialize(RHIInitInfo init_info)
    {
        ...
        // https://gcc.gnu.org/onlinedocs/cpp/Common-Predefined-Macros.html
#if defined(__linux__)
        char const* vk_layer_path = VULKAN_XSTR(VULKAN_VK_LAYER_PATH);
        setenv("VK_LAYER_PATH", vk_layer_path, 1);
#elif defined(__MACH__)
        // https://developer.apple.com/library/archive/documentation/Porting/Conceptual/PortingUnix/compiling/compiling.html
        char const* vk_layer_path    = VULKAN_XSTR(VULKAN_VK_LAYER_PATH);
        char const* vk_icd_filenames = VULKAN_XSTR(VULKAN_VK_ICD_FILENAMES);
        setenv("VK_LAYER_PATH", vk_layer_path, 1);
...
#error Unknown Compiler
    ...
    void VulkanRHI::createInstance()
    {
        ...
        appInfo.pApplicationName   = "VULKAN_renderer";
    ...
    VulkanManager::QueueFamilyIndices VulkanRHI::findQueueFamilies(VkPhysicalDevice physical_device) // for device and surface
    {
    ...
    VulkanManager::SwapChainSupportDetails VulkanRHI::querySwapChainSupport(VkPhysicalDevice physical_device)
    {
